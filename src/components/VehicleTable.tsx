...
export default function VehicleTable({ transactions: initialTransactions, currentUser }: VehicleTableProps) {
  ...
  const parseQuickAddText = (text: string) => {
    ...
    return { vehicle, vin, customer, contact, ref: '' }
  }
    ...
    <div className="relative">
      ...
      {showNotification && (
        <div className="fixed top-4 right-4 z-[100] animate-in slide-in-from-top-5 fade-in duration-300">
          <div className={`flex items-center gap-3 px-4 py-3 bg-white rounded-lg shadow-xl border border-gray-100 ${notificationType === 'error' ? 'border-l-4 border-l-red-500' :
            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-gray-200">
              <div className="p-6">
                ...
                <div className="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-gray-200 rounded-xl p-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">Ref (Optional):</label>
                    <input
                      type="text"
                      value={editableData?.ref || ''}
                      onChange={(e) => setEditableData(prev => prev ? { ...prev, ref: e.target.value } : null)}
                      className="w-full text-gray-900 bg-white p-3 rounded-lg border-2 border-gray-200 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="Manual Ref Entry"
                    />
                  </div>
